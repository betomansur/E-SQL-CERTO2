...
def main(args):
    ...
    print(len(dataset), " questions are loaded.")
    #dataset = dataset[0:501:20]
    #dataset = dataset[10:501:20]
    #dataset = dataset[5:501:20]
    #dataset = dataset[15:501:20]
    #dataset = dataset[1:501:20]
    #dataset = dataset[11:501:20]
    dataset = dataset[12:501:20]
    #dataset = dataset[5:6] # for testing purposes, only 10 questions are used
    
    for ind,t2s_object in enumerate(dataset):
        q_id = t2s_object["question_id"]
        if pipeline.pipeline_order == "CSG-SR":
            t2s_object_prediction, lista_tempo = pipeline.forward_pipeline_CSG_SR(t2s_object)

            t2s_object_prediction["timing"] = {
                "CSG": lista_tempo[0],
                "SR": lista_tempo[1],
                "total_time": sum(lista_tempo)
            }
        elif pipeline.pipeline_order == "CSG-QE-SR":
            t2s_object_prediction,lista_tempo = pipeline.forward_pipeline_CSG_QE_SR(t2s_object)
            t2s_object_prediction["timing"] = {
                "CSG": lista_tempo[0],
                "QE": lista_tempo[1],
                "SR": lista_tempo[2],
            ...
            t2s_object_prediction, lista_tempo = pipeline.forward_pipeline_SF_CSG_QE_SR(t2s_object)
            # Adiciona os tempos diretamente no dicionário da questão
            t2s_object_prediction["timing"] = {
                "SF": lista_tempo[0],
                "CSG": lista_tempo[1],
                "QE": lista_tempo[2],
        ...
        predicted_sql = t2s_object_prediction["SQL"]
    ...
    sf_list, csg_list, qe_list, sr_list, total_list = [], [], [], [], []

    for pred in predictions:
        if "timing" in pred:
            timing = pred["timing"]
            sf_list.append(timing.get("SF", 0))
            csg_list.append(timing.get("CSG", 0))
            qe_list.append(timing.get("QE", 0))
            sr_list.append(timing.get("SR", 0))
            total_list.append(timing.get("total_time", 0))
    ...
    media_tempos = {
        "avg_SF": avg(sf_list),
        "avg_CSG": avg(csg_list),
        "avg_QE": avg(qe_list),
        "avg_SR": avg(sr_list),
        "avg_total_time": avg(total_list)
    }

    # Imprime as médias
    print("Médias de tempo por etapa:")
    for etapa, valor in media_tempos.items():
        print(f"{etapa}: {valor:.2f} segundos")

    # Também pode adicionar isso no metrics.json:
    metric_object["tempo_medias"] = media_tempos
